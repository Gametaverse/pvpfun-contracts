...
describe("TokenVault 合约测试", function () {
    ...
    beforeEach(async function () {
        ...
        if (useWalletsFromEnv) {
            ...
            [owner, authorizer, claimer, otherAccount] = signers; // 使用前几个默认账户
        }

        // 2. 部署 MockERC20 (由 owner 部署并拥有)
        const MockERC20Factory = await ethers.getContractFactory("MockERC20", owner);
        mockToken = await MockERC20Factory.deploy("奖励代币", "RWD", owner.address);
        ...
        tokenVault = await TokenVaultFactory.deploy(authorizer.address);
        ...
        console.log(`已向 TokenVault (${tokenVaultAddress}) 提供 ${ethers.formatUnits(initialVaultSupply, 18)} RWD`);
    ...
    describe("部署与初始化", function () {
        ...
        it("Vault 应该持有初始的 RWD 代币", async function () {
    ...
    describe("claimReward 功能", function () {
        ...
        beforeEach(async function () {
            ...
            amount = ethers.parseUnits("100", 18); // 领取 100 RWD
        ...
        it("如果 Vault 余额不足以支付奖励，应该失败", async function () {
            ...
            // SafeTransfer 会失败
            await expect(tokenVault.connect(claimer).claimReward(claimData))
                .to.be.revertedWith("ERC20: transfer amount exceeds balance"); // 来自 SafeERC20
        });
    });
    ...
    describe("batchClaimReward 功能", function () {
        ...
        beforeEach(async function () {
            ...
            for (let i = 0; i < 2; i++) {
                ...
                const amount = ethers.parseUnits((10 + i).toString(), 18); // 10 RWD, 11 RWD
        ...
        it("如果 Vault 余额不足以支付列表中的某个奖励，整个批量交易应该回滚", async function () {
            ...
            await expect(tokenVault.connect(claimer).batchClaimReward(largeClaimList))
                .to.be.revertedWith("ERC20: transfer amount exceeds balance"); // 预计在第二次领取时失败
    ...
    describe("withdrawFunds 功能", function () {
        it("Owner 应该能提取 Vault 中的资金", async function () {
            const withdrawAmount = ethers.parseUnits("1000", 18); // 提取 1000 RWD
            ...
            // Owner 调用提现
            const tx = await tokenVault.connect(owner).withdrawFunds(mockToken.getAddress(), withdrawAmount);

            // 检查事件
            await expect(tx)
                .to.emit(tokenVault, "WithdrawFunds")
        ...
        it("非 Owner 不应该能提取资金", async function () {
            ...
            await expect(tokenVault.connect(otherAccount).withdrawFunds(mockToken.getAddress(), withdrawAmount))
                .to.be.revertedWithCustomError(tokenVault, "OwnableUnauthorizedAccount")
                .withArgs(otherAccount.address);
        });

        it("提取金额为 0 应该失败", async function () {
            await expect(tokenVault.connect(owner).withdrawFunds(mockToken.getAddress(), 0n))
                .to.be.revertedWith("PVP: amount must be greater than 0");
        });

        it("提取超过 Vault 余额的金额应该失败", async function () {
            const withdrawAmount = initialVaultSupply + 1n; // 比 Vault 总额还多
            await expect(tokenVault.connect(owner).withdrawFunds(mockToken.getAddress(), withdrawAmount))
